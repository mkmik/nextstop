...
pub fn draw_demo_chrome(_app: &App, p: &mut Painter) {
    ...
    for (i, n) in ["application-16", "file-code-16", "file-generic-16", "file-image-16", "file-text-16", "folder-16", "symlink-badge", "arrow", "resize-ns", "resize-nesw", "resize-nwse"].iter().enumerate() {
        p.icon(n, 500 + (i as i32 % 6) * 24, 190 + (i as i32 / 6) * 24, 16);
    }
    p.text(FontId::Regular, 12, 40, 290, &p.ellipsize_mid(FontId::Regular, 12, "a very long file name that needs truncating.txt", 120), BLACK);
    p.text(FontId::Regular, 12, 200, 290, &p.ellipsize(FontId::Regular, 12, "Applications and more text", 80), BLACK);
}
