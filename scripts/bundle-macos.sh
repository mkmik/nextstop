#!/bin/sh
# Build a release binary and wrap it in NeXTSTOP.app (+ .dmg). Run from the repo root on macOS.
set -eu
VERSION=$(grep '^version' Cargo.toml | head -1 | sed 's/.*"\(.*\)".*/\1/')
ARCH=$(uname -m); [ "$ARCH" = "arm64" ] || ARCH=x64
cargo build --release
APP=dist/NeXTSTOP.app
rm -rf dist && mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources" dist/icon.iconset
cp target/release/nextstop "$APP/Contents/MacOS/NeXTSTOP"
...
for px in 16 32 64 128 256 512 1024; do
  target/release/nextstop --render-icon "dist/icon.iconset/icon_${px}x${px}.png" --px "$px"
done
...
<plist version="1.0"><dict>
  <key>CFBundleName</key><string>NeXTSTOP</string>
  <key>CFBundleDisplayName</key><string>NeXTSTOP</string>
  <key>CFBundleIdentifier</key><string>com.nextstop.desktop</string>
  <key>CFBundleVersion</key><string>$VERSION</string>
  <key>CFBundleShortVersionString</key><string>$VERSION</string>
  <key>CFBundleExecutable</key><string>NeXTSTOP</string>
...
hdiutil create -volname NeXTSTOP -srcfolder "$APP" -ov -format UDZO "dist/NeXTSTOP-$VERSION-macos-$ARCH.dmg" >/dev/null
du -sh "$APP" dist/*.dmg
